...
public class PipeApplicationModel implements Serializable
{
    ...
    public TypeAction exptransAction;
    public TypeAction dettransAction;
    //public TypeAction unitransAction;
    //public TypeAction erltransAction;
    ...
    public ComponentAction startComponentAction;
    public ComponentAction firstComponentAction;
    public ComponentAction secondComponentAction;
    public ComponentAction thirdComponentAction;
    public ComponentAction forthComponentAction;
    public ComponentAction fifthComponentAction;
    ...
    public ModelGuideAction modelGuideAction;
    //添加组件的模块
    ...
    public PipeApplicationModel()
    {
        ApplicationSettings.register(this);

        _name = "基于Petri网的分布式仿真原型系统" ;
    ...
    private void registerActions()
    {
        ...
        modelGuideAction = new ModelGuideAction("建模向导     ","以向导的方式对航电系统进行Petri网建模", "G");
        componentAction = new ComponentAction("组件初始化","选择合适的组件", "ctrl G");
        ...
        exptransAction = new TypeAction("Exponential transition", Constants.EXPTRANS, "Add a exponential transition", "T", true);
        dettransAction = new TypeAction("Deterministic transition", Constants.DETTANS, "Add a deterministic transition", "T", true);
        //unitransAction = new TypeAction("Uniform transition", Constants.UNITRANS, "Add a uniform transition", "T", true);
        //erltransAction = new TypeAction("Erlang transition", Constants.ERLTRANS, "Add an erlang transition", "T", true);

        startComponentAction = new ComponentAction("start","初始化组件","K");
        firstComponentAction = new ComponentAction("first","添加first组件","K1");
        secondComponentAction = new ComponentAction("second","添加second组件","K2");
        thirdComponentAction = new ComponentAction("third","添加third组件","K3");
        forthComponentAction = new ComponentAction("forth","添加forth组件","K4");
        fifthComponentAction = new ComponentAction("fifth","添加fifth组件","K5");
    ...
    public void restoreMode()
    {
        ...
        exptransAction.setSelected(mode == Constants.EXPTRANS);
        dettransAction.setSelected(mode == Constants.DETTANS);
        //unitransAction.setSelected(mode == Constants.UNITRANS);
        //erltransAction.setSelected(mode == Constants.ERLTRANS);
    ...
    public void enableActions(boolean status, boolean pasteEnabled)
    {
        ...
        exptransAction.setEnabled(status);
        dettransAction.setEnabled(status);
        //unitransAction.setEnabled(status);
        //erltransAction.setEnabled(status);
