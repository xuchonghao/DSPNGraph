...
public class Token extends Observable implements Serializable
{
    ...
    public Token(String id, boolean enabled, int currentMarking, Color color)
    {
        ...
        ArrayList<IObserver> obsevers = new ArrayList<IObserver>();
        _color = color;
    }
                            ...
                            if(pn instanceof TransitionView)
                            {
                                ...
                                for(MarkingView token : arcView.getWeight())
                                {
                                    if(token.getToken().getID().equals(_id))
                                    {
                                        try
                                        {
                                            _forwardsIncidenceMatrix.set(placeNo, transitionNo, arcView.getWeightOfTokenClass(_id));
                                        }
                                        catch(Exception e)
                                        {
                            ...
                            if(pn instanceof TransitionView)
                            {
                                ...
                                for(MarkingView token : arcView.getWeight())
                                {
                                    if(token.getToken().getID().equals(_id))
                                    {
                                        try
                                        {
                                            _backwardsIncidenceMatrix.set(placeNo, transitionNo, arcView.getWeightOfTokenClass(_id));
                                        }
                                        catch(Exception e)
                                        {
