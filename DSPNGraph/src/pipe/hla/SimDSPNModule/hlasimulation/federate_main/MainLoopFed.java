package pipe.hla.SimDSPNModule.hlasimulation.federate_main;

import hla.rti1516.*;
import pipe.hla.SimDSPNModule.basemodel.OLGraph;
import pipe.hla.SimDSPNModule.basemodel.VexNode;
import pipe.hla.SimDSPNModule.hlasimulation.federate_main.auxiliary.*;
import pipe.hla.SimDSPNModule.hlasimulation.federate_main.event.IOEvent.EndEvent;
import pipe.hla.SimDSPNModule.hlasimulation.federate_main.event.IOEvent.FinishTokenReceivedEvent;
import pipe.hla.SimDSPNModule.hlasimulation.federate_main.event.IOEvent.FireTransitionEvent;
import pipe.hla.SimDSPNModule.hlasimulation.federate_main.event.IOEvent.GrantEvent;
...
public class MainLoopFed {
    ...
    public MainLoopFed(OLGraph olGraph, String federateName) throws RTIinternalError, UnknownHostException, ConnectException {
    ...
    public void run() throws RTIexception, ConnectException, UnknownHostException, MalformedURLException {
        ...
        //register synchronization points注册同步点
        System.out.println("Registering " + Constant._readyToPopulate + "...");
        barrier = new Barrier(Constant._readyToPopulate);
        _fedAmb.setSynchronizationPointRegistrationSucceededBarrier(barrier);
        //如果不指定同步集合，则该同步集合中包含当前所有已加入的所有联邦成员
        rti.registerFederationSynchronizationPoint(Constant._readyToPopulate, null);
        result = barrier.await();
        System.out.println("...registration succeeded.");

        System.out.println("Registering " + Constant._readyToRun + "...");
        barrier = new Barrier(Constant._readyToRun);
        _fedAmb.setSynchronizationPointRegistrationSucceededBarrier(barrier);
        rti.registerFederationSynchronizationPoint(Constant._readyToRun, null);
        result = barrier.await();
        System.out.println("...registration succeeded.");

        System.out.println("Registering " + Constant._readyToResign + "...");
        barrier = new Barrier(Constant._readyToResign);
        _fedAmb.setSynchronizationPointRegistrationSucceededBarrier(barrier);
        rti.registerFederationSynchronizationPoint(Constant._readyToResign, null);
        result = barrier.await();
        System.out.println("...registration succeeded.");
        ...
        while(!Constant._simulationEnds){
            ...
            do{//内部循环等待并分发回调函数，包含了时间推进请求GrantEvent、接收消息回调ReceivedInteractionEvent等等
                ...
                if (Constant._simulationEnds){
                    //_internalQueue.enqueue(new EndEvent(get_logicalTime(),this));
                    break timeLoop;
                }
        ...
        System.out.println("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~A运行时间是："+value);
    ...
    public void sendEndMessage(VexNode source,String targetName,int weight){
        //6、发送交互消息
    ...
    private void checkEndOfSimulation() {
        System.out.println("-------------------发生的次数为："+atomicInteger.get());
