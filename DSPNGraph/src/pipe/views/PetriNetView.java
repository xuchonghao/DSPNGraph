...
public class PetriNetView extends Observable implements Cloneable, IObserver, Serializable
{
    ...
    //private Graph graph;
    private ArrayOfGraph arrayOfGraph;//上海项目中的元素
    private OLGraph olGraph;

    public OLGraph getOlGraph() {
        return olGraph;
    }

    public void setOlGraph(OLGraph olGraph) {
        this.olGraph = olGraph;
    }
 ...
 */
    ...
    {
        if(transitionView != null)//肯定不等于null啊，前面已经判断过了
        {
            if(transitionView.isEnabled() && _placeViews != null)
            {
                ...
                for(int placeNo = 0; placeNo < _placeViews.size(); placeNo++)
                {
                    ...
                    {
                        TokenView tokenView = markingView.getToken();//感觉每一个markingView的getToken（）都是相同的
                        //猜想这个old。。。应该为0，指的是default的吧,第二个参数指的是 LinkedList<MarkingView> markviews
                        int oldMarkingPositionInTheList = positionInTheList(tokenView.getID(), _currentMarkingVector[placeNo]);
                        //System.out.println("oldMarkingPositionInTheList:" + oldMarkingPositionInTheList);
                        //得到当前库所中Default类型的token的数量
                        int oldMarking = _currentMarkingVector[placeNo].get(oldMarkingPositionInTheList).getCurrentMarking();
                        //得到需要增加某个库所的marking的数量
                ...
                //System.out.println("token发生后，使能的变迁："+Arrays.toString(result));
                TransitionView tra;   int type;
                        ...
                        if(type != 0){//非瞬时变迁
                            ...
                            for(int j=0;j<len;j++){//刚发生的变迁，或者和它同等的变迁
                                ...
                                if(arr[j] == 0 && tr.getType()!=0 && result[j] == true && preEnabledTransitions[j] == true){//将当前的arr保存到变迁中
                                    if(tr.getType() == 1){
                                        arr[j] = (1/tr.getRate()) * 1000;
                                        //System.out.println("当前的时延：" + arr[j]);
                                        //System.out.println("tr.getDelay():" + tr.getDelay());
                                    }
                    ...
                    //2、新使能  //重新使能的变迁进行新的赋值---TODO 这里也应该减去刚才发生的那一段时间ma?
                        ...
                        if(type == 1){//指数
                            if(tra.getDelay() > 0){//如果之前使能过
                                arr[i] = tra.getDelay();
                                //System.out.println("当前的时延：" + arr[i]);
                                //System.out.println("tra.getDelay():" + tra.getDelay());
                            }else{//之前没有使能过
                                arr[i] = (1 / tra.getRate()) * 1000;
                                 //System.out.println("当前的时延：" + arr[i]);
                                //System.out.println("tra.getDelay():" + tra.getDelay());
                            }
                        }else if(type == 2){//确定变迁
                            if(tra.getDelay() > 0){//如果之前使能过
                                ...
                                //System.out.println("tra.getRate():"+tra.getRate());
                                 //System.out.println("当前的时延：" + arr[i]);
                                //System.out.println("tr.getDelay():" + tra.getDelay());
    ...
    public void fireTransitionMarking(TransitionView transitionView){
    ...
    private ArrayList<ArcView> findOutArcs(TransitionView tr) {
        ArrayList<ArcView> arcViews = new ArrayList<>();
        for(int i=0;i<_arcViews.size();i++){
            ConnectableView source = _arcViews.get(i).getSource();
            if(source instanceof TransitionView && tr.equals(source) )
                arcViews.add(_arcViews.get(i));
        }
        return arcViews;
    }
    private ArrayList<ArcView> findInArcs(TransitionView tr) {
        ArrayList<ArcView> arcViews = new ArrayList<>();
        for(int i=0;i<_arcViews.size();i++){
            ConnectableView target = _arcViews.get(i).getTarget();
            if(target instanceof TransitionView && tr.equals(target))
                arcViews.add(_arcViews.get(i));
        }
        return arcViews;
    }

    public ArrayList<TransitionView> haveCycles(ArrayList<TransitionView> transitionViewList){
        boolean flag1 = false;boolean flag2 = false;
        ArrayList<TransitionView> deleteTr = new ArrayList<>();
         for(int m=0;m<transitionViewList.size();m++){
                for(int n=0;n<transitionViewList.size();n++){
                    if(n != m){
                        TransitionView tr1 = transitionViewList.get(m);
                        //找到与tr1为出发点的弧
                        ArrayList<ArcView> outTr1 = findOutArcs(tr1);
                        ArrayList<ArcView> inTr1 = findInArcs(tr1);
                        TransitionView tr2 = transitionViewList.get(n);
                        ArrayList<ArcView> outTr2 = findOutArcs(tr2);
                        ArrayList<ArcView> inTr2 = findInArcs(tr2);

                        for(int x=0;x<outTr1.size();x++)
                            for(int y=0;y<inTr2.size();y++)
                                if(outTr1.get(x).getTarget().equals(inTr2.get(y).getSource()))
                                   flag1 = true;

                         for(int x=0;x<outTr2.size();x++)
                            for(int y=0;y<inTr1.size();y++)
                                if(outTr2.get(x).getTarget().equals(inTr1.get(y).getSource()))
                                   flag2 = true;
        ...
        //环的话只发生一个
        for(int d=0;d<deleteTr.size();d++){
            LinkedList<TransitionView> linkedListtemp = new LinkedList<>(transitionViewList);
            linkedListtemp.remove(deleteTr.get(d));
            transitionViewList = new ArrayList<>(linkedListtemp);
        }
        return transitionViewList;
    }
    public void fireTransition2(ArrayList<TransitionView> transitionViewList)
    {
        //ArrayList<TransitionView> trueFiredTra = new ArrayList<>();
        double currentFiredTransitionDelay = transitionViewList.get(0).getDelay();
         if(transitionViewList.size() == 1){
                fireTransition(transitionViewList.get(0));
         }
        if(transitionViewList.size() > 1){//多个变迁，前面几个只发生token的变化，最后一个按照原来的发生（当然必须保证下一次该变迁仍然使能）
            transitionViewList = haveCycles(transitionViewList);
            int size = transitionViewList.size();//1个变迁的时候也符合
            while(size > 0){
                Random random = new Random();
                int listIndex = random.nextInt(size);
                TransitionView transitionView = transitionViewList.get(listIndex);
                //得到它的序号
                int transitionNo = _transitionViews.indexOf(transitionView);
                //判断是否使能
                LinkedList<MarkingView>[] markings = getCurrentMarkingVector();
                if(isTransitionEnabled(markings,transitionNo)){//循环后重新判断剩余的变迁是否使能
                    //trueFiredTra.add(transitionView);
                    //System.out.println("循环中token发生前，使能的变迁："+Arrays.toString(areTransitionsEnabled(markings)));
                    fireTransitionMarking(transitionView);//对随机的一个变迁只发生marking的变化
                    //System.out.println("--------这次fire的变迁是："+transitionView.getName()+","+transitionView.getDelay());
                    //System.out.println("循环中token发生后，使能的变迁："+Arrays.toString(areTransitionsEnabled(markings)));
                }
                //若使能了，把当前发生变迁删除；若循环后不使能，也要删除，只不过不发生marking的变化而已
                LinkedList<TransitionView> linkedList = new LinkedList<>(transitionViewList);
                linkedList.remove(transitionView);
                transitionViewList = new ArrayList<>(linkedList);
                //然后判断这时候的某些变迁是否使能，如果使能，继续，不是能，将其改掉或者删除
                //去掉这个发生的transitionView，长度也会减一
                size = transitionViewList.size();//每次都会变化
            }
            //token发生完毕，开始时间的变化//变迁发生、token发生后，计算时间的问题
                LinkedList<MarkingView>[] markings = getCurrentMarkingVector();
                int len = this._currentDelayMatrix.length;//长度和变迁的数量相同
                boolean[] result = new boolean[len]; double[] arr;
                //判断是否有新使能的变迁，还有当前变迁的delay也需要赋予一个新值
                result = areTransitionsEnabled(markings);//result要么全是瞬时变迁，要么全是非瞬时变迁
                //System.out.println("token发生后，使能的变迁："+Arrays.toString(result));
                TransitionView tra;   int type;
                for(int i=0;i<len;i++){//给pre。。。添加set和get方法
                    ...
                    //3、新不使能
                        ...
                        if(type != 0){//瞬时变迁，时延不发生变化
                            ...
                            //TODO 如果变为了0，还得把它重新赋值；前提得保证不是瞬时变迁，可能是多个发生了 1 1 1
                            for(int j=0;j<len;j++){//刚发生的变迁，或者和它同等的变迁
                                TransitionView tr = getTransition(j);//TODO 这里的for循环应该可以去掉
                                if(arr[j] == 0 && tr.getType()!=0 && result[j] == false && preEnabledTransitions[j] == true){//将当前的arr保存到变迁中
                                    if(tr.getType() == 1){
                                        arr[j] = (1/tr.getRate()) * 1000;
                                         //System.out.println("当前的时延：" + arr[j]);
    ...
    public TransitionView getRandomTransition()
    {
        ...
        // All the enabled _transitions are of the same type: 变迁发生规则
        ...
        if(isAllImmTransition(_transitionViews)){//此时的优先级是一样的，通过权重来选择一个瞬时变迁，这里用rate表示的weight
        ...
        }else{//2、不是瞬时变迁的情况下，选择 一个时延最短的变迁
            //int len = this._currentDelayMatrix.length;
            //System.out.println(Arrays.toString(_currentDelayMatrix));
            int index;
            double value;
           ...
           // System.out.println(a[0]);
            ...
            Arrays.sort(a);
           // System.out.print("使能变迁的时延分别为："+Arrays.toString(a));
            //找到合适的最小值
            value = minValue(a);
            //System.out.println(value);
            //找到对应的下标，可能有多个，需要进行一个选择//TODO 这里如果有多个相同的值
    ...
    //变迁发生规则
    public ArrayList<TransitionView> getRandomTransition2()
    {
        setEnabledTransitions();//将要使能的变迁设置为使能
        // All the enabled _transitions are of the same type: 变迁发生规则
        // a) all are immediate _transitions; 选择优先级最高的一个发生，若优先级相同，根据随机数来决定
        ...
        ArrayList enabledTransitions = new ArrayList();
        ArrayList<Double> ArrayOfEnabledTransitionDelay = new ArrayList<Double>();
        ArrayList<TransitionView> transitionViewsList = new ArrayList<>();
        double rate = 0;
        //1、都是瞬时变迁，靠优先级和权重（priority和weight）delay=0
        //2、    都是确定性变迁，靠delay
        //          都是指数变迁，靠rate---》转化为delay
        ...
        if(enabledTransitions.size() == 1)
        {
            transitionViewsList.add((TransitionView) enabledTransitions.get(0));
            return transitionViewsList;
        }
        //下面是指使能变迁的个数大于1的时候
        //1、全是瞬时变迁
        if(isAllImmTransition(_transitionViews)){//此时的优先级是一样的，通过权重来选择一个瞬时变迁，这里用rate表示的weight
           ...
           // System.out.println(a[0]);
            ...
            //找到对应的下标，可能有多个，需要进行一个选择//TODO 这里如果有多个相同的值
            indexList = findRandomIndex2(_currentDelayMatrix,value);
            for(int i=0;i<indexList.size();i++){
                transitionViewsList.add(this.getTransition(indexList.get(i)));
            }
            return transitionViewsList;
        }
        // no enabled transition found, so no transition can be fired
    ...
    private int findRandomIndex(double[] arr,double value){//当有多个相同的时候，随机选择一个返回
        ...
        for(int i=0;i<len;i++){
            if(arr[i] == value && _transitionViews.get(i).isEnabled()){//TODO
                list.add(i);
                //System.out.println("多个使能变迁的序号："+ i);
            }
        }
        int ll = list.size();
        if(ll == 1)
            ...
            Random r = new Random();
            int a = r.nextInt(ll);
           // System.out.println(getTransition(list.get(0)).getName() +" , "+ getTransition(list.get(1)).getName());
            return list.get(a);
        }
        else
            return -1;
    ...
    //TODO 这里返回可能有错误
    private ArrayList<Integer> findRandomIndex2(double[] arr,double value){//当有多个相同的时候，随机选择一个返回
        int len = arr.length;
        ArrayList<Integer> list = new ArrayList();
        for(int i=0;i<len;i++){
            if(arr[i] == value && _transitionViews.get(i).isEnabled()){//TODO
    ...
    public final boolean[] areTransitionsEnabled(LinkedList<MarkingView>[] markings)
    {
        ...
        int maxPriority = 0;
        //for(int t=0;t<_transitionViews.size();t++){
           // System.out.print(_transitionViews.get(t).getName()+",");
       // }
        for(int i = 0; i < transitionCount; i++)
        {
           // System.out.print(_transitionViews.get(i).getName()+",");
            ...
            if(result[i])
            {
                ...
                //System.out.println("使能的变迁："+t.getName()+","+t.getDelay());
                if(t.getType() != 0)
                {
                    hasTimed = true;
                }
    ...
    public void setEnabledTransitions()
    {
        ...
        //System.out.println(Arrays.toString(enabledTransitions));
        //同步变迁相对应的View类
        for(int i = 0; i < enabledTransitions.length; i++)
        {
            //System.out.print(_transitionViews.get(i).getId()+"");
            //System.out.println(enabledTransitions[i]+","+_transitionViews.get(i).getId()+","+_transitionViews.get(i).isEnabled());
            TransitionView transitionView = _transitionViews.get(i);
            if(enabledTransitions[i] != transitionView.isEnabled())
            {
                transitionView.setEnabled(enabledTransitions[i]);
            ...
            final int capacities[], final int placeCount, final int transitionCount)
    {
        ...
        for(int i = 0; i < transitionCount; i++)
        {
            ...
            for(int j = 0; j < placeCount; j++)
            {
                ...
                int totalCPlus = 0;
                int totalCMinus = 0;
                //System.out.println(" markings:"+markings.length);//System.out.println(" markings[j].size()):"+markings[j].size());//表示的是
                   ...
                   //if(m!=null){
                    ...
                    if(backwards)
                    {
                        ...
                        if(m!=null && m.getToken().getID().equals("Default")){
                           String s = _placeViews.get(j)._id;
                            //System.out.print(s+":");
                            //System.out.println(m.getCurrentMarking()+":"+CMinus[j][i]);
                            //printf(CMinus);
                            //printf(markings);
                         }
                   }
           ...
           // System.out.println(result[i]+","+transArray[i].getId());
            // we look for the highest priority of the enabled _transitions
            if(result[i])
            {
    ...
    public ArcView createArc(Element inputArcElement)
    {
       ...
       // System.out.println(inscriptionTempStorage);
