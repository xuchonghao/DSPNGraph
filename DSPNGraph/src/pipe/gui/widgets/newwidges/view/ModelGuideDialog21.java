 ...
 * 建模向导第2.5步，确定每个SPM中       分区的数量，其实是创建了SpmList
 *
 * 21直接跳到下一个页面为 每个发送端分区中链路条数 3页面
 */

public class ModelGuideDialog21 extends JDialog {
    ...
    public ModelGuideDialog21(java.awt.Frame parent, boolean modal, GuideModel guideModel) {
    ...
    private void initComponents() {
        ...
        jLabel2.setText("<html>请设置每个发送端系统SPM中分区的数量<html>");
    ...
    private void jButtonPrevStepActionPerformed(java.awt.event.ActionEvent evt) {
       ...
       /* int num = nowModel.getNumOfSPM();
        ...
        //直接让spm的list初始化
        this.nowModel.setSpmList(new LinkedList<SPM>());
        doClose(RET_CANCEL);
    ...
    private void jButtonNextStepActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        for(int i=0; i<num; i++)
        {
            SPM spm = new SPM();
            spm.setId(this.id[i]);
            spm.setParNum(ParNumber[i]);//分区的数量
            spmList.add(spm);
        }

        this.nowModel.setSpmList(spmList);
        doClose(RET_OK);
    	ModelGuideDialog3 guiDialog =  new ModelGuideDialog3(ApplicationSettings.getApplicationView(), true, this.nowModel);
        guiDialog.pack();
